...
class TopDownChargerUnc(BasePose):
    ...
    def __init__(self,
        ...
        if uncertainty_head is not None:
            uncertainty_head['train_cfg'] = train_cfg
            uncertainty_head['test_cfg'] = test_cfg

            if 'loss_keypoint' not in uncertainty_head and loss_pose is not None:
                warnings.warn(
                    '`loss_pose` for TopDown is deprecated, '
                    'use `loss_keypoint` for heads instead. See '
                    'https://github.com/open-mmlab/mmpose/pull/382'
                    ' for more information.', DeprecationWarning)
                uncertainty_head['loss_keypoint'] = loss_pose

            self.uncertainty_head = builder.build_head(uncertainty_head)
    ...
    def with_uncertainty(self):
        """Check if has uncertainty_head."""
        return hasattr(self, 'uncertainty_head')
    ...
    def forward_train(self, img, target, target_weight, img_metas, **kwargs):
        ...
        if self.with_uncertainty:
            output_uncertainty = self.uncertainty_head(features)
        ...
        if self.with_uncertainty:
            ...
            #     output, target, target_weight)
            # losses.update(uncertainty_accuracy)
    ...
    def forward_val(
        ...
        **kwargs
        ...
        if self.with_keypoint:
            ...
            result.update(keypoint_accuracy)
        ...
        if self.with_uncertainty:
            ...
            # uncertainty_accuracy = self.uncertainty_head.get_accuracy(
            #     output, target, target_weight)
            # result.update(uncertainty_accuracy)
            output_uncertainty = self.uncertainty_head.inference_model(
                features, flip_pairs=None
            )
            result["output_uncertainty"] = output_uncertainty
